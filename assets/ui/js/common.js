...
(function () {
  ...
  const ICONS = {
    ...
    'call-wave':      '<line x1="5" y1="10" x2="5" y2="14"/><line x1="9" y1="7" x2="9" y2="17"/><line x1="13" y1="4" x2="13" y2="20"/><line x1="17" y1="7" x2="17" y2="17"/><line x1="21" y1="10" x2="21" y2="14"/>',
  ...
  window.iconSvg = function (name, size, className) {
  ...
  window.mountSearchModal = function () {
      ...
      <div class="c-search-modal-mask" id="searchModal">
        <div class="c-search-modal">
          ...
          <div class="c-search-modal__body" id="searchModalBody">
            <button type="button" class="c-search-modal__item" id="searchModalNewChat">
              <span data-icon="edit"></span>
              <span class="c-search-modal__item__title">新聊天</span>
            </button>
            <div id="searchModalGroups"></div>
            <div class="c-search-modal__empty" id="searchModalEmpty" hidden>暂无对话记录</div>
          </div>
        </div>
      </div>`;
    ...
    input.addEventListener('input', function () {
      ...
      document.querySelectorAll('#searchModalBody .c-search-modal__item[data-conv-id]').forEach(function (el) {
        const txt = el.textContent.toLowerCase();
        el.style.display = (!kw || txt.includes(kw)) ? '' : 'none';
      });
      document.querySelectorAll('#searchModalGroups .c-search-modal__group-title').forEach(function (g) {
        let next = g.nextElementSibling;
        let anyVisible = false;
        while (next && !next.classList.contains('c-search-modal__group-title')) {
          if (next.classList.contains('c-search-modal__item') && next.style.display !== 'none') {
            anyVisible = true;
            break;
          }
      ...
      const emptyEl = document.getElementById('searchModalEmpty');
      if (emptyEl) {
        const hasVisible = Array.from(
          document.querySelectorAll('#searchModalBody .c-search-modal__item[data-conv-id]')
        ).some(function (el) { return el.style.display !== 'none'; });
        emptyEl.hidden = hasVisible || !kw;
      }
    });
  };
  ...
  /** @returns {{text:string,pct:number,depleted:boolean,hidden:boolean}} */
  window.quotaBarMetrics = function (item) {
    if (!item || !item.enabled) {
      return { text: '—', pct: 0, depleted: false, hidden: true };
    }
    if (item.limit <= 0) {
      return { text: '不限', pct: 100, depleted: false, hidden: false };
    }
  ...
  window.renderQuotaBars = function (container, quota) {
    if (!container) return;
    if (!quota) {
      container.innerHTML = '';
    ...
    rows.forEach(function (row) {
      var m = window.quotaBarMetrics(quota[row.key]);
      if (m.hidden) return;
      html +=
        '<div class="quota-bars__row' + (m.depleted ? ' is-depleted' : '') + '">' +
        '<div class="quota-bars__head"><span class="quota-bars__label">' + row.label + '</span>' +
        '<span class="quota-bars__value">' + m.text + '</span></div>' +
        '<div class="quota-bars__track"><div class="quota-bars__fill" style="width:' + m.pct + '%"></div></div>' +
        '</div>';
    });
    if (!html) {
      container.innerHTML = '';
